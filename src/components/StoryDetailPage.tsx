...
export function StoryDetailPage() {
  ...
  const storyI18n = t.storiesData[story.id]
  const classYear = storyI18n?.classYear ?? story.classYear
  const quote = storyI18n?.quote ?? story.quote
  const fullNarrative = storyI18n?.fullNarrative ?? story.fullNarrative
    ...
    <>
      ...
      <div className="hidden md:block px-6 md:px-12 max-w-7xl mx-auto mb-8 md:mb-12">
        <Link to="/stories" className="inline-flex items-center space-x-3 group">
          ...
          <span className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant group-hover:text-primary transition-colors">{t.stories_all_link}</span>
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-end">
          ...
          <div className="lg:col-span-7">
            <span className="font-label text-[10px] uppercase tracking-[0.3em] text-secondary mb-4 md:mb-6 block">
              {story.university} &middot; {classYear}
            </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-20">
          <div className="hidden lg:block lg:col-span-2">
            <div className="sticky top-32">
              <span className="font-label text-[10px] uppercase tracking-[0.2em] text-secondary block mb-4">{t.stories_narrative_label}</span>
              <div className="w-8 h-[1px] bg-secondary" />
            </div>
          </div>
          <div className="lg:col-span-8">
            <span className="lg:hidden font-label text-[10px] uppercase tracking-[0.2em] text-secondary block mb-6">{t.stories_narrative_label}</span>
            {fullNarrative.split('\n\n').map((paragraph, i) => (
        ...
        <div className="grid grid-cols-2">
          {prevStory ? (
            <Link to={`/stories/${prevStory.id}`} className="group py-10 md:py-16 pr-4 md:pr-12 border-r border-outline-variant/20">
              <span className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant mb-3 md:mb-4 block">{t.common_previous}</span>
          ...
          {nextStory ? (
            <Link to={`/stories/${nextStory.id}`} className="group py-10 md:py-16 pl-4 md:pl-12 text-right">
              <span className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant mb-3 md:mb-4 block">{t.common_next}</span>
      ...
      <section className="px-6 md:px-12 py-20 md:py-32 text-center bg-primary text-on-primary">
        <div className="max-w-3xl mx-auto">
          <h2 className="font-headline text-3xl md:text-5xl lg:text-6xl italic mb-8 md:mb-12 leading-tight">{t.stories_write_your_own}</h2>
          <p className="font-body text-zinc-400 text-sm md:text-lg mb-10 md:mb-16 max-w-xl mx-auto">{t.stories_write_your_own_body}</p>
