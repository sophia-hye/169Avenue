  ...
  return (
    <>
      ...
      <section className="px-6 md:px-16 lg:px-24 mb-24 md:mb-36 max-w-screen-2xl mx-auto pt-12 md:pt-16">
        <span className="inline-flex items-center gap-2 font-label text-[11px] uppercase tracking-[0.3em] text-violet-800 font-semibold bg-violet-100/70 px-3 py-1 mb-8">
      ...
      <section className="px-6 md:px-16 lg:px-24 mb-24 md:mb-36 max-w-screen-2xl mx-auto">
        <span className="font-label text-xs uppercase tracking-[0.3em] text-violet-700 font-semibold mb-4 block">
          {t.direction_who_tag}
        </span>
        <h2 className="font-headline text-3xl md:text-5xl text-primary tracking-tight mb-12 md:mb-16 max-w-3xl">
          {t.direction_who_title}
        </h2>
        <div className="grid grid-cols-1 gap-6 md:gap-8 max-w-2xl">
          {t.direction_who_items.map((item) => (
            <div
              key={item.label}
              className="bg-surface-container-lowest p-8 md:p-10 border-l-2 border-violet-500/60"
            >
              <span className="material-symbols-outlined text-violet-600 text-3xl mb-5 block">
                {item.icon}
              </span>
      ...
      <section className="bg-surface-container-low py-24 md:py-36 px-6 md:px-16 lg:px-24 mb-24 md:mb-36">
        <div className="max-w-4xl mx-auto">
          <h2 className="font-headline text-2xl md:text-4xl text-primary tracking-tight leading-snug mb-12 md:mb-14 max-w-3xl">
            {t.direction_problem_title}
          </h2>
          <ul className="space-y-6 md:space-y-8 mb-12">
            {t.direction_problem_items.map((q) => (
              <li
                key={q}
      ...
      <section className="px-6 md:px-16 lg:px-24 mb-24 md:mb-36 max-w-screen-2xl mx-auto">
        <span className="font-label text-xs uppercase tracking-[0.3em] text-violet-700 font-semibold mb-4 block">
          {t.direction_what_tag}
        </span>
        <h2 className="font-headline text-3xl md:text-5xl text-primary tracking-tight mb-12 md:mb-16 max-w-3xl">
          {t.direction_what_title}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {t.direction_what_items.map((item) => (
            <div
              key={item.label}
              className="bg-surface-container-lowest p-8 md:p-10"
            >
              <span className="material-symbols-outlined text-violet-600 text-3xl mb-6 block">
                {item.icon}
              </span>
      ...
      <section id="program-structure" className="scroll-mt-24 bg-surface-container-low py-24 md:py-36 px-6 md:px-16 lg:px-24 mb-24 md:mb-36">
        <div className="max-w-5xl mx-auto">
          <span className="font-label text-xs uppercase tracking-[0.3em] text-violet-700 font-semibold mb-4 block">
            {t.direction_structure_tag}
          </span>
          <h2 className="font-headline text-3xl md:text-5xl text-primary tracking-tight mb-12 md:mb-16 max-w-3xl">
            {t.direction_structure_title}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            {t.direction_structure_items.map((item) => (
              <div
                key={item.label}
                className="bg-surface p-8 md:p-10 flex items-start gap-6"
              >
                <span className="material-symbols-outlined text-violet-600 text-3xl shrink-0">
                  {item.icon}
                </span>
                <div>
      ...
      <section className="px-6 md:px-16 lg:px-24 mb-24 md:mb-36 max-w-4xl mx-auto">
        <span className="inline-block font-label text-[10px] uppercase tracking-[0.3em] text-violet-800 font-semibold bg-violet-100/70 px-3 py-1 mb-6">
          {t.direction_key_badge}
        </span>
        <h2 className="font-headline text-2xl md:text-4xl text-primary tracking-tight leading-snug mb-8">
          {t.direction_key_title}
        </h2>
        <p className="font-body text-base md:text-lg text-on-surface-variant leading-relaxed">
          {t.direction_key_body}
        </p>
      </section>
      ...
      <section className="bg-primary text-on-primary py-24 md:py-36 px-6 md:px-16 lg:px-24 mb-24 md:mb-36">
        <div className="max-w-5xl mx-auto">
          <span className="font-label text-xs uppercase tracking-[0.3em] text-violet-300 font-semibold mb-4 block">
            {t.direction_outcomes_tag}
          </span>
          <h2 className="font-headline text-3xl md:text-5xl tracking-tight mb-12 md:mb-16 max-w-3xl">
            {t.direction_outcomes_title}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            {t.direction_outcomes_items.map((item) => (
              <div
                key={item.step}
                className="bg-on-primary/5 border border-on-primary/10 p-8 md:p-10"
              >
                <span className="font-headline italic text-4xl text-violet-300/70 block mb-6">
                  {item.step}
                </span>
        ...
        >
          <div className="flex-1">
            ...
            <h3 className="font-headline text-xl md:text-2xl text-primary tracking-tight mb-2">
              {t.direction_next_link_label}
            </h3>
            <p className="font-body text-sm text-on-surface-variant leading-relaxed">
              {t.direction_next_link_desc}
            </p>
          </div>
...
export function DirectionTrackPage() {
  const { t } = useLanguage()
  usePageTitle(t.direction_page_title)
  return (
    <div className="bg-surface selection:bg-secondary/30">
      <div className="hidden md:block">
        <Navbar />
        <main className="pt-32"><DirectionContent /></main>
        <Footer />
      </div>
      <MobileShell activeTab="programs"><DirectionContent /></MobileShell>
      <MobileFooter />
    </div>
  )
}
