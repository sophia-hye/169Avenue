...
function DestinationsContent() {
  ...
  const REGIONS = (['us', 'uk', 'eu', 'ap'] as const).map((id, i) => ({
    id,
    title: t.destinations_items[i].title,
    tag: t.destinations_items[i].tag,
  }))

  return (
    <>
      <header className="px-6 md:px-16 max-w-screen-2xl mx-auto mb-10 md:mb-16">
        <span className="font-label text-xs uppercase tracking-[0.3em] text-secondary mb-4 block">{t.destinations_page_eyebrow}</span>
        <h1 className="font-headline text-4xl md:text-6xl lg:text-7xl text-primary tracking-tighter leading-none mb-4">
          {t.destinations_page_h1_1} <br /><span className="italic">{t.destinations_page_h1_2}</span>
        </h1>
        <p className="font-body text-on-surface-variant text-sm md:text-lg leading-relaxed max-w-lg mt-4">
          {t.destinations_page_intro}
        </p>
      </header>
        ...
        <section className="px-4 md:px-16 max-w-screen-2xl mx-auto mb-6" style={{ animation: 'fadeIn 0.3s ease' }}>
          <div className="bg-surface-container-low p-6 md:p-8">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <h3 className="font-headline text-xl md:text-2xl text-primary">{t.fields[selectedField.id]?.name ?? selectedField.name}</h3>
            </div>
            <p className="font-body text-on-surface-variant text-sm md:text-base max-w-3xl">{t.fields[selectedField.id]?.description ?? selectedField.description}</p>
          </div>
        </section>
      )}
            ...
            {REGIONS.map((r) => (
              <Link key={r.id} to={`/destinations/${r.id}`} className="group block bg-surface-container-low p-5 md:p-8 hover:bg-surface-container-high transition-colors duration-300">
                ...
                <div className="flex items-center space-x-1 md:space-x-2 mt-2 md:mt-6">
                  <span className="font-label text-[9px] md:text-[10px] uppercase tracking-widest text-on-surface-variant">{t.common_explore}</span>
...
export function DestinationsPage() {
  const { t } = useLanguage()
  usePageTitle(t.destinations_page_title)
  return (
