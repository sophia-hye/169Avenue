...
function ImageGallery({ images, nationName }: { images: readonly string[]; nationName: string }) {
  const count = images.length

  if (count === 2) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-16">
        <GalleryImage src={images[0]} alt={`${nationName} 1`} className="aspect-[16/10]" />
        <GalleryImage src={images[1]} alt={`${nationName} 2`} className="aspect-[16/10]" />
      </div>
    )
  }
  ...
  if (count === 3) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-16">
        <GalleryImage src={images[0]} alt={`${nationName} 1`} className="md:col-span-2 md:row-span-2 aspect-[4/3] md:aspect-auto md:h-full" />
        <GalleryImage src={images[1]} alt={`${nationName} 2`} className="aspect-[16/10]" />
        <GalleryImage src={images[2]} alt={`${nationName} 3`} className="aspect-[16/10]" />
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-16">
      <GalleryImage src={images[0]} alt={`${nationName} 1`} className="md:col-span-3 aspect-[21/9]" />
      <GalleryImage src={images[1]} alt={`${nationName} 2`} className="aspect-[4/3]" />
      {images[2] && <GalleryImage src={images[2]} alt={`${nationName} 3`} className="aspect-[4/3]" />}
      {images[3] && <GalleryImage src={images[3]} alt={`${nationName} 4`} className="aspect-[4/3]" />}
    </div>
  )
}
...
export function UKRegionDetailPage() {
  ...
  const nationsWithUnis = getNationsWithUniversities()
  const currentIdx = nationsWithUnis.indexOf(id)
  const prevNation = currentIdx > 0 ? nationsWithUnis[currentIdx - 1] : undefined
  const nextNation = currentIdx < nationsWithUnis.length - 1 ? nationsWithUnis[currentIdx + 1] : undefined

  const pageContent = (
    <>
      {/* Back */}
      <div className="hidden md:block px-6 md:px-16 max-w-screen-2xl mx-auto mb-8 md:mb-12">
        <Link to="/destinations/uk" className="inline-flex items-center space-x-3 group">
      ...
      {/* Nation Overview: Images + Description */}
      <section className="px-6 md:px-16 max-w-screen-2xl mx-auto mb-12 md:mb-20">
        <ImageGallery images={nationDetail.images} nationName={nationName} />
